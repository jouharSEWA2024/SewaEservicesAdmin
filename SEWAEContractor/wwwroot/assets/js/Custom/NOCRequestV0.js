$(document).ready(function () {
    ...
    if (ServiceTypeId == 4) {
        if (AccountList == "R1" || AccountList == "R2") {
            $('#ProjectPurpose').show();
        }
        else {
            $('#ProjectPurpose').hide();
...
function LoadServiceTypeActions() {
    var SubService = $('#SubServiceTypeList').val();
    if (SubService != "" || SubService != null || SubService != undefined) {
        $('#SRVerification input[name="ExtraParameter"]').remove();
        $('<input>').attr({ type: 'hidden', name: 'CurrentStage', value: 'SubServiceTypeSelection'}).appendTo('#SRVerification');
        $(SRVerification).submit();
    }
}
function FirstSubmit() {
    $('#SRVerification input[name="ExtraParameter"]').remove();
    $('<input>').attr({ type: 'hidden', name: 'CurrentStage', value: 'FirstSubmit' }).appendTo('#SRVerification');
    $(SRVerification).submit();
...
function SecondarySubmit() {
    ...
    if (SubServiceTypeId == 16 || SubServiceTypeId == 17 || SubServiceTypeId == 18 || SubServiceTypeId == 19) {
    ...
    if (IsMainContractorRequired == "True" && CheckMainContractor=="False") {
